...
/* toast types :
normal = black bg, white font
ok = green bg, whitefont
ko = orange bg, white font
*/
...
extension UIView {
    ...
    */
    ...
    func makeToast(message msg: String, duration: Double, position: AnyObject, title: String, type: String) {
        var toast = self.viewForMessage(msg, title: title, image: nil, type: type)
        self.showToast(toast: toast!, duration: duration, position: position)
    }
    
    func makeToast(message msg: String, duration: Double, position: AnyObject, image: UIImage, type:String) {
        var toast = self.viewForMessage(msg, title: nil, image: image, type:type)
        self.showToast(toast: toast!, duration: duration, position: position)
    }
    
    func makeToast(message msg: String, duration: Double, position: AnyObject, title: String, image: UIImage, type:String) {
        var toast = self.viewForMessage(msg, title: title, image: image, type:type)
        self.showToast(toast: toast!, duration: duration, position: position)
    }
    ...
    func makeToastActivity(type:String) {
        self.makeToastActivity(position: HRToastActivityPositionDefault, type:type)
    }

    func makeToastActivityWithMessage(message msg: String, type:String){
        self.makeToastActivity(position: HRToastActivityPositionDefault, message: msg, type:type)
    }
    
    func makeToastActivity(position pos: AnyObject, message msg: String = "", type:String) {
		var existToast = objc_getAssociatedObject(self, &HRToastView) as! UIView?
		        if existToast != nil {
		            if let timer = objc_getAssociatedObject(existToast, &HRToastTimer) as? NSTimer {
		                timer.invalidate();
    ...
    */
    ...
    func viewForMessage(msg: String?, title: String?, image: UIImage?, type:String) -> UIView? {
